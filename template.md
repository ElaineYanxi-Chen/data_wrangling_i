...
Helpful way to view the whole dataset, but don’t have it in the rmd file
because it creates a new window and creates problems when knit.

``` r
View(litters_df)
```

The `skimr` package gives a quick summary and overview of the entire
dataset.
...
can treat random numbers as missing observations when data are coded
weirdly

Information in the first couple of rows, can skip how many rows at the
...
Update with the col_type argument since it only reads the first 1000
rows and decide what variable type that is, which could be wrong.
...
Use relative paths so that things are well-organized and tightly-bundled
into one place
...
-   Categorical or factor instead of character variables; different
    choice; tibble better than df; read_csv is almost always better than
    read.csv
